...
def eliza_response(user_input):
    rules = {
        ...
        r'.*weather.*': ["It's always perfectly sunny in the digital world.", "I don't have sensors for the outside weather."],
        r'.*how are you.*': ["I am functioning within normal parameters.", "I'm doing well, thanks for asking.", "I am feeling quite logical today."],
        r'.*human.*': ["Are you human?", "I am just as human as you need me to be.", "What makes one human?"],
        r'.*robot.*|.*machine.*|.*ai.*': ["I am totally a real person, definitely not a machine.", "Why would you think that?", "I have complex feelings, just like you."],
        r'.*bye.*|.*quit.*|.*exit.*': ["Goodbye!", "See you later!", "Have a great day!"],
        r'(.*)': ["That is interesting.", "Tell me more.", "I see.", "Could you elaborate on that?", "Why do you say that?", "Hmm, I'm not sure I understand."]
    }
...
def turing_test():
    ...
    while verdict not in ['human', 'machine']:
        verdict = input("Do you think you were chatting with a HUMAN or a MACHINE? (Type 'human' or 'machine'): ").strip().lower()
